...
class ReservaHistorialIntegrationTest extends IntegrationTestSupport {
    ...
    void historial_devuelveSoloLasReservasDelPasajeroLogueado() throws Exception {
        ...
        mockMvc.perform(get("/api/reservas").header("Authorization", "Bearer " + pasajero1))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.length()").value(1));

        mockMvc.perform(get("/api/reservas").header("Authorization", "Bearer " + pasajero2))
                .andExpect(status().isOk())
    ...
    void cancelarReserva_devuelveElStockAlVuelo() throws Exception {
        ...
        String reservaJson = mockMvc.perform(post("/api/carrito/checkout").header("Authorization", "Bearer " + pasajero))
                .andExpect(status().isCreated())
                .andReturn().getResponse().getContentAsString();
        Long reservaId = objectMapper.readTree(reservaJson).get("id").asLong();

        mockMvc.perform(post("/api/reservas/" + reservaId + "/cancelar").header("Authorization", "Bearer " + pasajero))
                .andExpect(status().isOk())
    ...
    void cancelarReserva_deOtroPasajero_devuelve404() throws Exception {
        ...
        String reservaJson = mockMvc.perform(post("/api/carrito/checkout").header("Authorization", "Bearer " + dueno))
                .andReturn().getResponse().getContentAsString();
        Long reservaId = objectMapper.readTree(reservaJson).get("id").asLong();

        String otro = registrarYLoguearPasajero("pasajerointruso");

        mockMvc.perform(post("/api/reservas/" + reservaId + "/cancelar").header("Authorization", "Bearer " + otro))
                .andExpect(status().isNotFound());
        mockMvc.perform(get("/api/reservas/" + reservaId).header("Authorization", "Bearer " + otro))
                .andExpect(status().isNotFound());
    }
}
